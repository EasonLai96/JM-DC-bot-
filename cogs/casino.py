...
# 🆕 共用版本：原本 casino.py 自己有一份 _safe_respond，現在統一放在 utils.py
from utils import safe_respond as _safe_respond


class Casino(commands.Cog):
    # 🆕 這個 dict 只該記「目前正在連輸的人」，不該變成永久的使用者名冊
    MAX_LOSS_STREAK_ENTRIES = 2000

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # 紀錄玩家的連輸場數，用於「平庸之福流」補償判定 {user_id: consecutive_losses}
        # 🛠️ 修復：值本身只會在 0~2 之間（達到 3 就歸零），但「key」是每一位賭過的
        # 玩家，而且永遠不會被移除 —— 只贏過一次的玩家也會留下一筆 0。這裡在每次
        # 累加時順手修剪掉值為 0 的舊紀錄，讓它的大小只跟「當下連輸中的人數」有關。
        self.loss_streaks = {}
        # 🔒 紀錄目前正在進行賭局（已扣款但尚未結算）的玩家，防止旋轉動畫等待期間被重複觸發造成併發扣款漏洞
        self.active_players = set()

    def _bump_loss_streak(self, uid: int) -> int:
        """連輸次數 +1，回傳新值；必要時修剪掉已經歸零的舊紀錄。"""
        self.loss_streaks[uid] = self.loss_streaks.get(uid, 0) + 1
        if len(self.loss_streaks) > self.MAX_LOSS_STREAK_ENTRIES:
            for k in [k for k, v in self.loss_streaks.items() if v <= 0]:
                self.loss_streaks.pop(k, None)
        return self.loss_streaks[uid]
    ...
    async def casino_dice(self, interaction: discord.Interaction, 數量: int, 貨幣類型: Literal["萌力值 (MP)", "次元結晶 (DC)"]):
        ...
        try:
            ...
            # 1. 🛠️ 修復：原本是「檢查餘額」跟「只有輸了才扣款」分成兩個各自獨立的
            # 步驟，中間隔著整場擲骰子的動畫與判定邏輯，理論上這段期間如果同一使用者
            # 從別的管道（例如 /power_plant）搶先扣走了 MP/DC，這裡最後扣款時
            # 會被 add_mp/add_dc 內建的 max(0, ...) 保護夾成 0，而不是正確地擋下這次下注。
            # 改成一開始就用 try_spend() 原子性地「檢查＋預扣」本金，之後贏了再把
            # 本金＋淨利一起加回去，經濟效果跟原本完全一樣，只是把扣款動作提前、
            # 而且變成單一原子操作，杜絕中途被搶先扣款的競爭窗口。
            is_mp = (貨幣類型 == "萌力值 (MP)")
            success, _ = await profile_store.try_spend(
                uid,
                mp_cost=數量 if is_mp else 0,
                dc_cost=0 if is_mp else 數量,
            )
            if not success:
                cur_bal = p["deposit"] if is_mp else p["dimension_crystal"]
                unit = "MP" if is_mp else "DC"
                await interaction.followup.send(f"❌ 你的{'萌力值 (MP)' if is_mp else '次元結晶 (DC)'}不足！當前餘額：`{cur_bal:,}` {unit}")
                return
            ...
            # 3. 結算邏輯
            # 🛠️ 修復：本金已經在上面用 try_spend() 原子性預扣過了，所以這裡的
            # 輸/贏/平手邏輯要相應調整——贏的時候要把「本金＋淨利」一起加回去
            # （總經濟效果跟修復前完全一樣，只是扣款時機提前了），平手則要把
            # 本金原封不動地退回（修復前因為本金從未被扣過，平手不需要做任何事；
            # 現在本金已經被預扣，平手必須主動退款，不然本金會憑空消失）。
            if p_total > b_total:
                # 玩家獲勝：抽稅 5%
                ...
                if 貨幣類型 == "萌力值 (MP)":
                    await profile_store.add_mp(uid, 數量 + win_net)  # 本金 + 淨利
                    ...
                    await profile_store.add_dc(uid, 數量 + win_net)  # 本金 + 淨利
                
                self.loss_streaks[uid] = 0
                embed.color = discord.Color.green()

            elif p_total < b_total:
                # 玩家失敗（本金已在預扣階段扣除，這裡不用再扣一次）
                if 貨幣類型 == "萌力值 (MP)":
                    embed.description = f" 📉 **大失敗！** 損失了 **`{數量:,}` MP**。輸掉的資金已化為虛無..."
                
                    # 平庸之福流 連輸安慰補償判定
                    if flow == "平庸之福流":
                        self._bump_loss_streak(uid)
                    ...
                    # 結晶走私流 特權判定：本金已被預扣，這裡改成觸發時「退還」本金
    ...
    async def casino_wheel(self, interaction: discord.Interaction, 下注mp: int):
        ...
        try:
            ...
            # 🛠️ 修復：原本是先用 get_profile() 檢查一次餘額，再另外呼叫 add_mp(-下注mp)
            # 扣款，兩步之間隔了 await，理論上有極小機率被其他管道搶先扣款。
            # 改用 try_spend() 把「檢查＋扣款」包進同一次原子操作。
            success, _ = await profile_store.try_spend(uid, mp_cost=下注mp)
            if not success:
    ...
    async def casino_blackjack(self, interaction: discord.Interaction, 下注mp: int):
        ...
        # 🛠️ 修復（玩家被永久鎖住）：get_profile() / try_spend() 若丟例外（例如
        # profiles.json 損毀、讀寫失敗），舊版會直接往外拋，uid 就永遠留在
        # active_players 裡——該玩家之後所有 /dice、/wheel、/blackjack 都會被
        # 「你目前還有一局賭局正在結算中」擋掉，而且**沒有任何超時會清掉它**，
        # 只能重啟 bot 才能解除。這裡保證任何失敗路徑都會釋放這個鎖。
        try:
            p = await profile_store.get_profile(uid)
            flow = self._check_profession_flow(p["profession"])
        except Exception as e:
            self.active_players.discard(uid)
            log.error(f"❌ [21點] 讀取玩家資料失敗，已釋放鎖 {uid}: {e}")
            await _safe_respond(interaction, "⚠️ 讀取你的資料時發生錯誤，請稍後再試一次！")
            return

        # 🛠️ 修復：原本是「檢查餘額」與「扣款」分開兩步，中間隔著 await，
        # 改用 try_spend() 把「檢查＋扣款」包進同一次原子操作。
        try:
            success, _ = await profile_store.try_spend(uid, mp_cost=下注mp)
        except Exception as e:
            self.active_players.discard(uid)
            log.error(f"❌ [21點] 扣款失敗，已釋放鎖 {uid}: {e}")
            await _safe_respond(interaction, "⚠️ 扣款時發生錯誤，請稍後再試一次！")
            return
        ...
        try:
                ...
                async def end_game(self, inter, status, msg_text, color):
                    async with self._settle_lock:
                        ...
                        if status == "win":
                            ...
                            # 玩家輸了
                            if self.flow == "平庸之福流":
                                self.cog._bump_loss_streak(self.user_id)
            ...
            # 🛠️ 修復：本金已於上方扣除，若這段中途發生任何未預期例外，
            # 直接把下注金額退還給玩家，並釋放 active_players 鎖，
            # 避免「錢被扣走但賭局面板送不出去」或「永久卡在賭局中」的情況。
            await profile_store.add_mp(uid, 下注mp)
            self.active_players.discard(uid)
            log.error(f"❌ [21點] 開局失敗，已自動退還本金 {下注mp:,} MP 給玩家 {uid}: {e}")
            try:
                if interaction.response.is_done():
                    await interaction.followup.send("⚠️ 開局時發生未預期錯誤，本金已全額退還，請重新嘗試一次！", ephemeral=True)
                else:
                    await interaction.response.send_message("⚠️ 開局時發生未預期錯誤，本金已全額退還，請重新嘗試一次！", ephemeral=True)
            except Exception:
                pass
