...
BACKUP_DIR = os.path.join(DATA_DIR, "backups")   # 🆕 每日快照存放處
BACKUP_KEEP = 7                                  # 🆕 只保留最近 7 份
...
def _prune_backups() -> None:
    """只保留最近 BACKUP_KEEP 份快照，避免在雲伺服器上無限累積。"""
    try:
        names = sorted(
...
def _maybe_daily_backup() -> None:
    """每天第一次覆蓋存檔前，先把「磁碟上目前的版本」複製一份快照。

    🆕 為什麼需要：profiles.json 是整個經濟系統的單一檔案。_save_raw 雖然有
    tmp+replace（不會寫出半截檔），但**完全沒有歷史版本**——任何一次意外
    （程式 bug 誤刪、手滑覆蓋、磁碟異常）都是全部玩家資料歸零且無法補救。
    這裡保留最近 BACKUP_KEEP 天的快照，成本只有一天一次檔案複製。

    ⚠️ 必須在覆蓋「之前」呼叫，快照才會是變更前的狀態。
    重啟後 _last_backup_date 會重置，但下方 os.path.exists 檢查讓它保持冪等，
    不會覆蓋當天已經存在的快照。
    """
    ...
    try:
        ...
        snapshot_path = os.path.join(BACKUP_DIR, f"profiles-{today}.json")
        if not os.path.exists(snapshot_path):
            shutil.copy2(PROFILES_PATH, snapshot_path)
            log.info(f"🗄️ [ProfileStore] 已建立每日備份快照: {os.path.basename(snapshot_path)}")
        _prune_backups()
    except Exception as e:
        # 備份失敗絕不能影響正常的存檔流程
        log.warning(f"⚠️ [ProfileStore] 建立每日備份快照失敗（不影響本次存檔）: {e}")


def _save_raw(data: dict):
    try:
        _maybe_daily_backup()   # 🆕 覆蓋前先留一份當天的快照
...
async def get_profile(user_id: int) -> dict:
    ...
    async with _lock:
        ...
        # 🛠️ 修復（顯示與扣款不一致）：原本這段只改記憶體裡的 dict、**從來沒有存檔**，
        # 但 try_spend / execute_upgrade / execute_reclaim 走的是 _ensure_schema（只補
        # 缺少的欄位、不套用這個遷移）→ 老玩家的名片顯示 5000 MP，實際扣款路徑卻只看到
        # DEFAULT_PROFILE 補上的 100，變成「看到餘額不足卻永遠花不掉卡片上的資產」。
        # 這裡在遷移真的發生時就落地存檔，讓兩邊看到同一個數字。
        migrated = False
...
async def profile_exists(user_id: int) -> bool:
    """檢查某位玩家是否已有資料（**唯讀，不會建立記錄**）。

    🆕 為什麼需要：`get_profile()` 在找不到資料時會「順手建立」一筆預設記錄，而
    `/profile` 允許輸入任意 UID 做跨伺服器查詢 —— 任何人只要枚舉 snowflake 就能替
    不存在的使用者灌入大量垃圾記錄，而 profiles.json 每次經濟操作都是整檔 load+dump，
    檔案成長會拖慢所有指令。
    """
    async with _lock:
        data = _load_raw()
...
async def admin_set_level(user_id: int, delta: int, cap: Optional[int] = None) -> tuple:
    """【管理員用】原子地把玩家等級增減 `delta`，夾在 1~cap 之間。

    🆕 為什麼需要：`/debug_give` 原本是「鎖外用 get_profile 讀 old → 鎖外算 new →
    鎖內寫入」，owner 執行時若同時有玩家升級，玩家的變動會被這個舊值覆蓋
    （典型 read-modify-write lost update），而且雙方都不會看到任何錯誤。

    回傳 `(old_level, new_level)`；找不到該玩家的資料時回傳 `(None, None)`
    （刻意不建立記錄，避免打錯 UID 就生出垃圾資料）。
    """
    cap = cap or LEVEL_CAP
    async with _lock:
        data = _load_raw()
        uid_str = str(user_id)
        record = data.get(uid_str)
        if record is None:
            return None, None
        _ensure_schema(record)
        try:
            old = int(record.get("sign_level", 1) or 1)
        except (TypeError, ValueError):
            old = 1
        new = max(1, min(cap, old + delta))
        record["sign_level"] = new
        _save_raw(data)
        return old, new
...
async def try_spend(user_id: int, mp_cost: int = 0, dc_cost: int = 0) -> tuple:
    """
    🆕 原子扣款：在「同一次」鎖的持有期間內完成「檢查餘額」與「扣款」，
    修復原本各處常見的「先 get_profile() 檢查一次餘額，再另外呼叫 add_mp/add_dc
    扣款」寫法——因為這兩步之間隔了至少一次 await，理論上同一使用者在極短時間內
    從不同指令/管道同時觸發，有機會在餘額檢查通過「之後」才被另一筆扣款搶先扣掉，
    導致這筆扣款被 add_mp/add_dc 內建的 max(0, ...) 保護夾成 0，而不是正確地
    回報「餘額不足，本次操作失敗」。

    可以同時檢查/扣除 MP 與 DC 兩種貨幣（例如升級、補簽都是兩種一起扣），
    只要任一種餘額不足，就完全不會扣款（兩種貨幣都不動），並回傳 (False, 目前資料)。
    成功時回傳 (True, 扣款後的最新玩家資料)。
    """
    async with _lock:
        data = _load_raw()
        uid_str = str(user_id)
        if uid_str not in data:
            data[uid_str] = DEFAULT_PROFILE.copy()
            data[uid_str]["last_hourly_time"] = time.time()
        _ensure_schema(data[uid_str])

        cur_mp = data[uid_str].get("moe_point", 0)
        cur_dc = data[uid_str].get("dimension_crystal", 0)

        if cur_mp < mp_cost or cur_dc < dc_cost:
            return False, data[uid_str]

        new_mp = cur_mp - mp_cost
        new_dc = cur_dc - dc_cost
        data[uid_str]["moe_point"] = new_mp
        data[uid_str]["deposit"] = new_mp
        data[uid_str]["dimension_crystal"] = new_dc
        data[uid_str]["sha_coin"] = new_dc
        _save_raw(data)
        return True, data[uid_str]
...
async def contribute_to_power_plant(user_id: int, amount: int) -> dict:
    ...
    🛠️ 修復（憑空生幣）：舊版用 `await add_mp(user_id, -amount)` 扣款，但 add_mp
    內部是 `max(0, current + amount)`——餘額不足時會被「夾成 0」而不是擋下這筆操作，
    然而發電廠仍然照全額 amount 計入。也就是說只要呼叫端檢查餘額之後、真正扣款之前
    有餘額變動（其他指令、賭場結算…），玩家就能「沒付錢也注入」，
    直接製造 MP 並可能觸發全服 buff。

    改用 try_spend() 做原子性「檢查＋扣款」：餘額不足就整筆失敗、完全不計入發電廠。
    回傳值新增 "ok" 欄位，讓呼叫端能分辨成功與失敗（預設 True 以相容舊呼叫端）。
    """
    success, profile = await try_spend(user_id, mp_cost=amount)
    if not success:
        g = _load_global()
        return {
            "ok": False,
            "current": g.get("total_contributed", 0),
            "target": PP_TARGET,
            "triggered": False,
            "balance": profile.get("moe_point", 0),
        }
    ...
    async with _lock:
        ...
        _save_global(g)
        return {"ok": True, "current": g["total_contributed"], "target": PP_TARGET, "triggered": triggered}

def _ensure_schema(user_data: dict) -> dict:
    """✅ 修復缺失欄位：確保舊資料在被直接操作前，一定含有 DEFAULT_PROFILE 的所有欄位
    (KeyError 根因：部分函式繞過 get_profile() 直接讀寫 raw data，
     若欄位是後來才加進 DEFAULT_PROFILE 的，舊玩家資料裡不會有它，直接 += 1 就會炸掉)"""
    for key, val in DEFAULT_PROFILE.items():
        if key not in user_data:
            user_data[key] = val
    return user_data
...
async def execute_upgrade(user_id: int, target_lvl: int, mp_cost: int, dc_cost: int) -> bool:
    """
    扣除升級費用並提升階級。
    🛠️ 修復：原本是「扣 MP」「扣 DC」「存檔更新等級」三個各自獨立上鎖的步驟，
    呼叫端（economy.py）在呼叫這個函式之前是另外用 get_profile() 檢查一次餘額，
    中間有數次 await，理論上有極小機率被其他扣款搶先。
    現在把「檢查餘額＋扣款＋升級」全部包進同一次鎖的持有期間內完成，
    餘額不足時完全不會有任何欄位被修改，並回傳 False 讓呼叫端知道要顯示失敗訊息。
    """
    async with _lock:
        data = _load_raw()
        uid_str = str(user_id)
...
async def execute_reclaim(user_id: int, mp_cost: int, dc_cost: int) -> bool:
    """
    執行時空扭轉補簽扣費，並更新總補簽次數。
    🛠️ 修復：同 execute_upgrade，把「檢查餘額＋扣款＋更新補簽次數」包進同一次鎖的
    持有期間內完成，避免呼叫端另外檢查餘額與這裡扣款之間出現競爭條件。
    餘額不足時完全不會扣款，並回傳 False。
    """
    async with _lock:
        data = _load_raw()
        uid_str = str(user_id)
