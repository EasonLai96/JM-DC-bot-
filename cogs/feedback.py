...
class FeedbackModal(discord.ui.Modal):
    def __init__(self, category: str, bot: commands.Bot, attachment_url: Optional[str] = None):
        ...
        # 🛠️ 修復：Discord embed 的 field value 上限是 **1024 字元**，而這裡原本允許
        # 輸入到 1500 字，加上 ``` 外框之後必定超標 → Discord 直接回 400，
        # 整則回饋連送都送不出去（使用者只會看到「發送失敗，技術錯誤」）。
        # 收斂到 1000 字，確保加了 fence 之後仍在 1024 以內。
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 🛠️ 修復：把內容裡的 ``` 中和掉，避免使用者輸入的內容撐破外層 code block
        # （logger_config.py 早就有同樣的處理，這裡原本漏了）。
        # 同時再做一次硬性截斷，保證 field value 絕不超過 1024 字元。
        safe_content = str(self.fb_content.value or "").replace("```", "`\u200b``")
