...
class AdminCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self._last_status = None  # 記錄上一次的 (伺服器數, 用戶數)，用來判斷是否真的有變動
    ...
    async def update_status(self):
        ...
        # 🛠️ 修復：原本每 10 分鐘無論有沒有變化都會寫一次 log，
        # 洗版洗到真正的錯誤訊息很難從一堆重複的「狀態同步」裡找出來。
        # 改成只有伺服器數或用戶數真的變動時才記錄一次。
        current_status = (total_guilds, total_members)
        if current_status != self._last_status:
            log.info(f"📊 [狀態同步] 狀態有變動，已更新：{status_text}")
