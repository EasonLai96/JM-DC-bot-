...
class DiscordRelayHandler(logging.Handler):
    ...
    # 🆕 緩衝區硬上限：即使因為任何原因 flush 迴圈沒在跑，記憶體也不會無限成長
    ...
    def emit(self, record):
        ...
        #
        # 🛠️ 修復（記憶體洩漏）：這個 handler 是在 setup_logger() 裡「無條件」掛上
        # logger 的，但唯一會清空 _buffer 的 _flush_loop() 只有在 set_target() 被
        # 呼叫時才會建立。也就是說：只要主程式從未設定轉發頻道（docstring 也說
        # 「預設沒有設定頻道」），每一筆 log 都會永久堆在 _buffer 裡清不掉，
        # 在 1GB RAM 的機器上跑久了必然 OOM。
        # 沒有轉發目標時直接丟棄最省事，也符合「沒有要轉發」的語意。
        ...
        with self._lock:
            ...
            # 第二層保險：即使 flush 迴圈卡住或還沒啟動，也最多只留 MAX_BUFFER_LINES 行
            if len(self._buffer) > self.MAX_BUFFER_LINES:
                del self._buffer[:-self.MAX_BUFFER_LINES]
